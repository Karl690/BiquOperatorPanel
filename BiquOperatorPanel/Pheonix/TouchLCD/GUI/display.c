...
void Format_Int32(uint8_t row, void* info)
{
	...
	GUI_DrawString(LEFT_PADDING, Y,varInfo->Label, varInfo->Color_1);
	sprintf(strTempVal, "%d ", (int)(*((uint32_t*)varInfo->VariablePointer)));
	GUI_DrawString(VALUE_POS, Y, strTempVal, varInfo->Color_2);
}
...
void Format_Hex8(uint8_t row, void* info)
{
	...
	GUI_DrawString(LCD_WIDTH / 2, Y, strTempVal, varInfo->Color_2);
...
void Format_Ascii(uint8_t row, void* info)
{
	varInfo = (LcdVariableInfo*)info;
	uint16_t Y = PADDING + (LCD_LINESIZE * row);
	GUI_DrawString(LEFT_PADDING, Y, varInfo->Label, varInfo->Color_1); //label?
...
void Format_MemoryDumpHex(uint8_t row, void* info)
{
	...
	uint16_t Y = PADDING + (LCD_LINESIZE * row);
	GUI_DrawString(LEFT_PADDING, Y, strTempVal, varInfo->Color_2);
}
...
void Format_Bar_Dutty(uint8_t row, void* info)
{
	...
	GUI_DrawString(LEFT_PADDING, PADDING,varInfo->Label, varInfo->Color_1);
	...
	GUI_DrawString((X1 + X3)/2, Y, strTempVal, varInfo->Color_1);
...
void Format_Bar_Status(uint8_t row, void* info)
{
	...
	GUI_DrawString((X1 + X2)/2, PADDING, strTempVal, varInfo->Color_2);
}
...
void Format_Bar_Float(uint8_t row, void* info)
{
	...
	GUI_DrawString((X1 + X3) / 2 - 10, Y, strTempVal, varInfo->Color_2);
}
...
void Format_SoapStringWithIndex(uint8_t row, void* info)
{
	...
	varInfo = (LcdVariableInfo*)info;
	GUI_DrawString(LEFT_PADDING, PADDING, varInfo->Label, varInfo->Color_1); //label?
	GUI_DrawString(VALUE_POS, PADDING, strTempVal, varInfo->Color_2);
}
