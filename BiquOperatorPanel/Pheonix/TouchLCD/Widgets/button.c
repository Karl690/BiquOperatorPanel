...
void button_on_paint(Button* button, Point posParent)
{
	...
	GUI_FillRect(pos.x, pos.y, pos.x + button->Size.width, pos.y + button->Size.height, button->BackgroundColor);
	if (button->BorderWidth > 0)
	{
		GUI_DrawRect(pos.x, pos.y, pos.x + button->Size.width, pos.y + button->Size.height, button->BorderColor);
	}
	GUI_DrawString(pos.x, pos.y, button->Text, button->ForegroundColor);
