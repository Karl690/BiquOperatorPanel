...
{
	secs->Stream = (uint8_t)(buf[3] % 128);
	secs->Function = (uint8_t)buf[4];
	secs->RepleyExpected =  (buf[3] & 0x80) == 0 ? 0 : 1;
	...
	while (i < len)
	{
		...
		if (num > 0)
		{
			uint8_t* val_bytes = (uint8_t*)malloc(num);
			memcpy(val_bytes, buf + i, num); // copy the value 
			if (format == SECS_LIST)
			{
			...
			{
				item = (SecsItem*)malloc(sizeof(SecsItem));
				item->Format = format;
				item->Count = num;
				item->RawData = val_bytes;
				if (list == NULL)
				{
					list = (List*)malloc(sizeof(List));
					list_init(list);
...
void ConvertSecsBinaryToStringList(uint8_t* buf, char* stringList)
{
	uint16_t row = 0;
		...
		if (num > 0)
		{
			...
			case SECS_U8:
				...
				for (int k = 0; k < num; k+= 8)
				{
					...
					sprintf(values, "%d", format == SECS_I8 ? *(int64_t*)endian : *(int64_t*)endian);
					strcat(row_string, values);
				}
				strcat(row_string, ">");
				if (isList) list_num--;
			...
			case SECS_U4:
				...
				for (int k = 0; k < num; k += 4)
				{
					...
					sprintf(values, "%d", format == SECS_I4 ? *(int32_t*)endian : *(int32_t*)endian);
					strcat(row_string, values);
				}
				strcat(row_string, ">");
				if (isList) list_num--;
			...
			case SECS_F4:
				...
				for (int k = 0; k < num; k += 4)
				{
					...
					memcpy(endian, buf + k + i, 4);
					SWAP(endian, 4); //big to little.
					sprintf(values, "%d", format == SECS_I8 ? *(float*)endian : *(float*)endian);
			...
			case SECS_F8:
				...
				for (int k = 0; k < num; k += 8)
				{
					...
					memcpy(endian, buf + k + i, 8);
					SWAP(endian, 8); //big to little.
					sprintf(values, "%d", format == SECS_I8 ? *(long*)endian : *(long*)endian);
					strcat(row_string, values);
				}
				strcat(row_string, ">");
				if (isList) list_num--;
